...
export async function POST(request) {
  try {
    ...
    // Curățăm textul de caractere non-standard care pot cauza erori la trimitere.
    const cleanedTextContent = textContent.replace(/[^\x00-\x7F]/g, "");

    const genAI = new GoogleGenerativeAI(process.env.GOOGLE_API_KEY);
    const model = genAI.getGenerativeModel({ model: "gemini-1.5-pro-latest" });
    
    // Folosim textul curățat (cleanedTextContent) în promptul final
